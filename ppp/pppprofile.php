						...
						<tbody>
							...
							?>
								<td style='text-align:center;'>
									<?php 
									// FIX: Proteksi profile bawaan mikrotik & penambahan urlencode untuk AJAX hapus
									if ($pname == "default" || $pname == "default-encryption") {
										echo "<i class='fa fa-minus-square text-muted' title='Default profile cannot be deleted'></i>";
									} else {
										echo "<a href='./?remove-pprofile=" . urlencode($pid) . "&pname=" . urlencode($pname) . "&session=" . $session . "' onclick=\"return confirm('Are you sure to delete profile (" . $pname . ")?')\"><i class='fa fa-minus-square text-danger' title='Remove " . $pname . "'></i></a>";
									}
									?>
									</td>
									<?php
								// FIX: Memperbaiki double tanda sama dengan pada link edit profile
								echo "<td><a title='Open User Profile " . $pname . "' href='./?ppp=edit-profile&id=" . urlencode($pid) . "&session=" . $session . "'><i class='fa fa-edit'></i> " . $pname . "</a></td>";
								echo "<td>" . $local_address . "</td>";
