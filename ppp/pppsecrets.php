						...
						<tbody>
							...
							?>
								<td style='text-align:center;'>
									<!-- FIX: Menambahkan urlencode pada ID agar AJAX hapus sukses -->
									<i class='fa fa-minus-square text-danger pointer' onclick="if(confirm('Are you sure to delete secret (<?= $sname; ?>)?')){loadpage('./?remove-pppsecret=<?= urlencode($sid); ?>&rempname=<?= $sname; ?>&session=<?= $session; ?>')}else{}" title='Remove <?= $sname; ?>'></i>&nbsp;&nbsp;&nbsp;
									<?php
									echo "<a title='Open secret by profil " . $sname . "'  href='./?ppp=profiles&profile=" . $profile . "&session=" . $session . "'><i class='fa fa-users'></i></a>&nbsp;&nbsp;&nbsp;";
									if ($sdisabled == "true") {
										$sprocess = "'./?enable-pppsecret=" . urlencode($sid) . "&secretsheduler=" . $sname . "&session=" . $session . "'";
										echo '<span class="text-warning pointer" title="Enable User ' . $sname . '"  onclick="loadpage(' . $sprocess . ')"><i class="fa fa-lock "></i></span></td>';
									} else {
										$sprocess = "'./?disable-pppsecret=" . urlencode($sid) . "&secretsheduler=" . $sname . "&session=" . $session . "'";
										echo '<span class="pointer" title="Disable User ' . $sname . '"  onclick="loadpage(' . $sprocess . ')"><i class="fa fa-unlock "></i></span></td>';
									}
									?>
								</td>
							<?php
								// FIX: Penyesuaian parameter link edit agar sinkron dengan template modifikasinya
								echo "<td><a title='Open User secret " . $sname . "' href='./?ppp=edit-secret&id=" . urlencode($sid) . "&schedulerbyname=" . $sname . "&session=" . $session . "'><i class='fa fa-edit'></i> $sname</a></td>";
								echo "<td>" . $password . "</td>";
								echo "<td>" . $service . "</td>";
								echo "<td>" . $callerid . "</td>";
								echo "<td>" . $profile . "</td>";
								echo "<td>" . $localaddress . "</td>";
								echo "<td>" . $remoteaddress . "</td>";
